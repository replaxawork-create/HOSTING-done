    ...
    for package in requirements:
        try:
            ...
            try:
                ...
                # Note: print() outputs are not sent to Telegram, so no pe() needed here
...
# Maps standard Unicode emoji → (Telegram animated custom emoji ID, fallback char).
# Applied to all bot.send_message / bot.reply_to text and captions.
# Inline keyboard button labels are intentionally NOT processed here.
#
# IDs are from Telegram's built-in animated emoji sticker set (tg://emoji?id=…).
PREMIUM_EMOJI_MAP = {
    # ── Status / feedback ────────────────────────────────────────────────────
    "✅": ("5368324170671202286", "✅"),   # animated checkmark
    "❌": ("5447644880824181073", "❌"),   # animated cross
    "⚠️": ("5467910814641080739", "⚠️"),  # warning
    "⚠":  ("5467910814641080739", "⚠️"),
    # ── Common UI ────────────────────────────────────────────────────────────
    "🔥": ("5413879192267805083", "🔥"),
    "👋": ("5373123633415723521", "👋"),
    "🆔": ("5440539497383087970", "🆔"),
    ...
    "⏹": ("5436040291042091267", "⏹"),
    "🔁": ("5435885070526058575", "🔁"),
    "▶️": ("5438241991042091267", "▶️"),
    "▶":  ("5438241991042091267", "▶️"),
    # ── Files / data ─────────────────────────────────────────────────────────
    "📦": ("5451882646321438073", "📦"),
    "📝": ("5413183324681216512", "📝"),
    "📄": ("5440882646321438073", "📄"),
    "🗑": ("5471905112678091267", "🗑"),
    "⬅️": ("5438082912456091267", "⬅️"),
    "⬅":  ("5438082912456091267", "⬅️"),
    # ── Stats / system ───────────────────────────────────────────────────────
    "👥": ("5373041458095140148", "👥"),
    "🚀": ("5413109808584820050", "🚀"),
    "💾": ("5440882646321411999", "💾"),
    "⏰": ("5436040291042267250", "⏰"),
    "🔍": ("5413879192267805093", "🔍"),
    "⚙️": ("5467737286141050873", "⚙️"),
    "⚙":  ("5467737286141050873", "⚙️"),
    "📈": ("5440882646321411920", "📈"),
}

def replace_with_premium_emoji(text: str) -> str:
    """Replace standard emoji in *text* with Telegram <tg-emoji> custom emoji tags.
    Safe to call multiple times; already-replaced tags are not double-processed.
    Inline keyboard button labels should never be passed here."""
    if not text:
        return text
    for emoji_char, (emoji_id, fallback) in PREMIUM_EMOJI_MAP.items():
...
def pe(text: str) -> str:
    """Shorthand for replace_with_premium_emoji — wrap any message string with pe()."""
    return replace_with_premium_emoji(text)
# ─────────────────────────────────────────────────────────────────────────────

BOT_TOKEN = "8612025095:AAGBsd2LspnhCGYsLuQB28orZ0m7kxQ-uuU"

# Group join settings
GROUP_INVITE_LINK = "https://t.me/+tlzrClEeAIBjZWU1"
GROUP_ID = -1003958917606
...
def is_user_in_group(user_id):
    ...
    if GROUP_ID == 0:
        return True  # Skip check if GROUP_ID not set
    try:
...
def send_join_message(chat_id):
    """Send join group message with inline buttons"""
    kb = InlineKeyboardMarkup()
    kb.row(
        InlineKeyboardButton("Join 🔗", url=GROUP_INVITE_LINK),
        InlineKeyboardButton("Verify ✅", callback_data="verify_join")
    )
    bot.send_message(
        chat_id,
        "👋 <b>Join our group chat to continue</b> ✅\n\n"
        "1️⃣ Click <b>Join</b> to join our group\n"
        "2️⃣ Click <b>Verify</b> after joining",
        reply_markup=kb
    )

def send_welcome(message):
    """Send the actual welcome after join verified"""
...
👇 Use buttons below to get started!
    ...
    gif_path = os.path.join(BASE_DIR, "welcome.mp4")
    ...
    if os.path.exists(gif_path):
        try:
            ...
            logger.error(f"Failed to send welcome GIF: {e}")
...
def updates_handler(message):
    bot.send_message(message.chat.id, pe("🗨️ Join our group chat: https://t.me/+tlzrClEeAIBjZWU1"))

@bot.message_handler(func=lambda m: m.text == "📞 Contact Owner")
def contact_handler(message):
    bot.send_message(message.chat.id, pe("📞 Contact: t.me/infinity1x"))
...
def send_files_list(chat_id, user_id):
    files = list_user_files(user_id)
    if not files:
        bot.send_message(chat_id, pe("📁 <b>Your Files</b>\n\nNo files uploaded yet."))
        return
    
    text = pe("📁 <b>Your Files</b>\n\nClick on a file to manage it:")
    kb = InlineKeyboardMarkup()
...
def document_handler(message):
    ...
    if len(user_files) >= MAX_FILES_PER_USER:
        bot.reply_to(message, pe(f"❌ You've reached the file limit ({MAX_FILES_PER_USER}). Delete some files to upload new ones."))
        return
    ...
    try:
        ...
        bot.reply_to(message, pe(f"❌ Failed to download file: {str(e)}"))
        return
    ...
    try:
        ...
        bot.reply_to(message, pe(f"❌ Failed to save file: {str(e)}"))
        return
    ...
    if file_type in ["zip", "archive"]:
        bot.reply_to(message, pe("📦 Extracting archive..."))
        ...
        if not success:
            bot.reply_to(message, pe(f"❌ Failed to extract archive: {error}"))
            try:
                os.remove(file_path)
            except:
                pass
        ...
        if not main_file:
            bot.reply_to(message, pe("❌ No main Python or JS file found in archive."))
            try:
        ...
        bot.reply_to(message, pe(f"✅ Found main file: {os.path.basename(main_file)}"))
    ...
    if file_type in ["python", "javascript"] and extracted_dir is None:
        bot.reply_to(message, pe(f"✅ File uploaded! Starting automatically..."))
        start_file_process(file_id, message.chat.id)
    else:
        bot.reply_to(message, pe(f"✅ File uploaded! Use 'My Files' to manage it."))
...
def start_file_process(file_id, chat_id):
    ...
    if should_stop:
        bot.send_message(chat_id, pe(f"⚠️ Cannot start: {reason}"))
        return
    
    file_record = get_file_record(file_id)
    if not file_record:
        bot.send_message(chat_id, pe("❌ File not found"))
        return
    ...
    if os.path.isdir(file_path):
        ...
        if not main_file:
            bot.send_message(chat_id, pe("❌ No main file found in directory"))
            return
        ...
        if not os.path.exists(file_path):
            bot.send_message(chat_id, pe(f"❌ File not found: {file_path}"))
            return
    ...
    if ext == ".py":
        ...
        if os.path.exists(requirements_path):
            bot.send_message(chat_id, pe("📦 Installing requirements.txt..."))
            success, message = install_requirements_from_file(requirements_path, chat_id, original_name)
            if not success:
                bot.send_message(chat_id, pe(f"⚠️ Requirements installation had issues: {message}"))
            else:
                bot.send_message(chat_id, pe(f"✅ Requirements installed: {message}"))
        
        # Install missing imports
        bot.send_message(chat_id, pe("🔍 Checking for missing imports..."))
        imports = extract_imports(target_file)
        if imports:
            success, message = install_missing_imports(imports, chat_id, original_name)
            bot.send_message(chat_id, pe(f"📦 Import check: {message}"))
    ...
    if ext == ".py":
        ...
        cmd = [node_path, target_file]
    else:
        bot.send_message(chat_id, pe(f"❌ Unsupported file type: {ext}"))
        return
    ...
    try:
        ...
        error_msg = pe(f"❌ Failed to start process: {str(e)}")
...
def callback_handler(call):
    ...
    try:
        if data.startswith("manage:"):
            ...
            bot.send_message(chat_id, pe("⏹ Process stopped"))
            time.sleep(1)
            show_file_management(chat_id, file_id, user_id, call.message.message_id)
            ...
            bot.send_message(chat_id, pe("🗑 File deleted"))
            send_files_list(chat_id, user_id)
            ...
            log_text = pe(f"📄 <b>Logs for {html_lib.escape(file_name)}</b>") + f"\n\n<pre>{html_lib.escape(logs)}</pre>"
...
def show_file_management(chat_id, file_id, user_id, message_id=None):
    ...
    if file_record["user_id"] != user_id:
        bot.send_message(chat_id, pe("❌ Access denied"))
        return
...
if __name__ == "__main__":
    logger.info("Starting 24x7 Infinity Hosting Bot...")
    start_bot()